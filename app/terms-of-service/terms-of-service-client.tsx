...
interface TermsOfServiceSection { title: string; content: string; order: number; }
interface TermsOfServiceData {
  ...
  agreementSection: { description: string; lastUpdated: string; };
}

interface TermsOfServiceClientProps { schemaJsonLd?: object | null; headScripts?: string; bodyEndScripts?: string; customCss?: string; }

export default function TermsOfServiceClient({ schemaJsonLd, headScripts, bodyEndScripts, customCss }: TermsOfServiceClientProps) {
        ...
        <section className="relative py-20 bg-gradient-to-br from-blue-600 via-blue-700 to-blue-800 overflow-hidden">
          ...
          <div className="relative container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto text-center">
              <h1 className="text-3xl md:text-5xl font-bold text-white tracking-tight mb-6">{data?.heroSection.heading || "Terms & Conditions"}</h1>
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto">
              <div className="prose prose-lg max-w-none">
                <div className="mb-12">
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">{data?.companyInfo.websiteUrl ? `${data.companyInfo.websiteUrl} Terms of Service` : "Clensy LLC Terms of Service"}</h2>
                  <p className="text-gray-700 mb-6">{data?.agreementSection.description || "By booking any service with Clensy LLC ('Clensy Cleaning'), either through our website, over the phone, or by email/text, you agree to comply with these Terms & Conditions."}</p>
                  <p className="text-sm text-gray-500">Last Updated: {data?.agreementSection.lastUpdated}</p>
                </div>
