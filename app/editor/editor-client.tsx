...
interface EditorClientProps {
  strapiUrl: string;
  pageBuilderApiKey: string;
  strapiClientToken: string;
}
...
function EditorPageContent({ strapiUrl, pageBuilderApiKey, strapiClientToken }: EditorClientProps) {
  ...
  if (!pageBuilderApiKey || pageBuilderApiKey.trim() === '') {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-md">
...
STRAPI_PAGE_BUILDER_API_KEY=your_api_key_here
  ...
  const strapiSettings = {
    url: strapiUrl,
    authToken: strapiClientToken || undefined,
    imageUrl: strapiUrl,
    locale: 'en',
  };
...
export default function EditorClient({ strapiUrl, pageBuilderApiKey, strapiClientToken }: EditorClientProps) {
  return (
    <Suspense
      fallback={
        <div className="flex items-center justify-center min-h-screen">
